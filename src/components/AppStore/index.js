...
class AppStore extends Component {
  ...
  render() {
    ...
    const searchResult = filteredProjects.filter(each =>
      ...
      <div className="container">
        ...
        <ul className="list-container">
          {tabsList.map(eachTab => (
            <TabItem
              itemDetails={eachTab}
              key={eachTab.appId}
              updateTabElement={this.updateTabElement}
              isActive={activeTabId === appsList.tabId}
            />
          ))}
        </ul>
        ...
        <ul className="list-container">
          {searchResult.map(eachApp => (
            <AppItem appItems={eachApp} key={eachApp.appId} />
          ))}
        </ul>
      </div>
    )
